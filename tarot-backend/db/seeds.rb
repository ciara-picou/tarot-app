...
#   Character.create(name: 'Luke', movie: movies.first)
    ...
    cards.each do | card |
        if card[:"Affirmation"] && card[:"Astrology"]
            c = Card.create(
                name: card[:name],
                number: card[:number],
                arcana: card[:arcana],
                ...
                hebrew_alphabet: card[:'Hebrew Alphabet'],
                numerology: card[:'Numerology'],
                elemental: card[:'Elemental'],
                mythical_spiritual: card[:'Mythical/Spiritual'],
                astrology: card[:'Astrology'],
                affirmation: card[:'Affirmation']
            )
        elsif card[:"Affirmation"]
            c = Card.create(
                name: card[:name],
                number: card[:number],
                arcana: card[:arcana],
        ...
        card[:fortune_telling].each do | f |
            FortuneTelling.create(
                card_id: c.id,
                content: f
            )
        end
        card[:keywords].each do | k |
            Keyword.create(
                card_id: c.id,
                content: k
            )
        end
        card[:'Questions to Ask'].each do | q |
            QuestionsToAsk.create(
                card_id: c.id,
                content: q
            )
        end
        card[:meanings][:light].each do | m |
            Meaning.create(
                card_id: c.id,
                light_shadow: 'light',
                content: m
        ...
        card[:meanings][:shadow].each do | m |
            Meaning.create(
                card_id: c.id,
                light_shadow: 'shadow',
                content: m
